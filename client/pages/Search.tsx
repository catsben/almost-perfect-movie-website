...
export default function Search() {
  ...
  const filteredResults = searchResults?.results?.filter((result: SearchResult) => {
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary">
              {filter === 'all' ? 'All Results' : filter === 'movie' ? 'Movies' : 'TV Shows'}
            </Badge>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
            {filteredResults.map((result: SearchResult) => {
              const item = {
                ...
                first_air_date: result.release_date || result.first_air_date || '',
